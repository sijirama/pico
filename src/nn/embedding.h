...
/*
  ...
  so our return is gonna be the input indices, but for each we've gotten our embedding vector - a
  bettrer representation [input_indices x embed_vector/embed_dim]
  */
static inline struct PicoTensor *pico_embedding_apply(
    struct PicoContext *ctx, struct PicoEmbedding *embedding, struct PicoTensor *input_indices) {
...
static inline struct PicoEmbedding *
pico_embedding_init(struct PicoContext *ctx, int num_embeddings, int embedding_dim) {
    ...
    // input_indices: are indices of tokens, and the output will be a tensor containing their
